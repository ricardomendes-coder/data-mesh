...
def dashboards_index(
    ...
    context = _shell_context(
        ...
        ai_enabled=ai.enabled() and bool(settings.assis_dashboard_agent_key or settings.ai_provider != "assis"),
        first_chart={},
...
_TILE_WIDTHS = {"full": 12, "half": 6, "third": 4}
...
async def dashboard_generate(
    ...
    """Build a whole dashboard from a prompt: the AI plans the charts (query +
    viz + mapping + width), grounded in the catalog; we create each chart and the
    dashboard, auto-flow the tiles on the grid, and open it in the editor. Broken
    queries still become tiles — they show their error there, to fix or remove."""
    if not store.available():
        return JSONResponse({"error": "The app database is not configured."}, status_code=503)
    if not access.allows(store.FEATURE, "dashboard_builder"):
        return JSONResponse({"error": "Você não tem acesso ao editor de painéis."}, status_code=403)
    if not ai.enabled():
        return JSONResponse({"error": "A IA não está configurada."}, status_code=400)
    try:
        body = await request.json()
        ...
        body = {}
    prompt = str((body or {}).get("prompt") or "").strip()[:1000]
    if not prompt:
        return JSONResponse({"error": "Descreva o painel que você quer."}, status_code=400)

    catalog = _assistant_catalog(prompt, access, limit=12)
    if not catalog:
        return JSONResponse({"error": "Nenhum dataset acessível para montar o painel."}, status_code=400)
    try:
        plan = ai.plan_dashboard(prompt, catalog)
    except ai.AIError as exc:
        return JSONResponse({"error": str(exc)}, status_code=502)
    if not plan.get("charts"):
        return JSONResponse({"error": "A IA não conseguiu montar gráficos para esse pedido."}, status_code=422)

    catalog_db = settings.datasets_database
    dash = store.save_dashboard(store.Dashboard(
        slug=store.unique_slug(plan["title"], exists=lambda s: store.get_dashboard(s, with_items=False)),
        title=plan["title"],
        created_by=user,
    ))
    ...
    x = y = row_h = 0
    created = 0
    for spec in plan["charts"][:8]:
        chart = store.save_chart(store.Chart(
            slug=store.unique_slug(spec["title"]),
            title=spec["title"],
            source_db=catalog_db,
            sql=spec["sql"],
            chart_type=spec["chart_type"] if spec["chart_type"] in charts.CHART_TYPE_KEYS else "bar",
        ...
        if x + w > 12:
            ...
            row_h = 0
        store.add_item(
            dash.slug, chart.slug,
            width=spec["width"] if spec["width"] in store.WIDTHS else "half",
    ...
    logger.info("Dashboard %r generated by %s (%d charts) from a prompt", dash.slug, user, created)
    return JSONResponse({
        "ok": True,
        "url": str(request.url_for("dashboard_edit", slug=dash.id)),
        "title": dash.title,
        "created": created,
    })
